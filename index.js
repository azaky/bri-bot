...
const updateScore = async () => {
  try {
    ...
    const ours = scoreboards.map(scoreboard => scoreboard.find(team => team.name === 'K2IV'));
    console.log(ours);
      ...
      if (last) {
        const updates = ours.map((result, i) => {
          ...
          if (result.rank < last[i].rank) {
            changes.push(`You moved up the leaderboard from **rank ${last[i].rank}** to **rank ${result.rank}**!`);
          }
          if (result.rank > last[i].rank) {
            changes.push(`You moved down the leaderboard from **rank ${last[i].rank}** to **rank ${result.rank}** 😔`);
          }
          if (result.score > last[i].score) {
            changes.push(`Your score improved from **${last[i].score}** to **${result.score}**!`);
          }
          if (result.score < last[i].score) {
            changes.push(`Your score decreased from **${last[i].score}** to **${result.score}** ... but ... how ...?`);
          }
          if (changes.length > 0) {
